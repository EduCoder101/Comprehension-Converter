...
let testType = 'reading'; // 'reading' or 'language-conventions'
...
function loadSettings() {
  ...
  const sheetsUrl = localStorage.getItem('tg_sheets_url') || localStorage.getItem('comprehension_hub_sheet_url') || '';
...
function validateSheetsUrl() {
  ...
  const help = document.getElementById('sheetsUrlHelp');
  if (url && url.includes('docs.google.com/spreadsheets')) {
    help.innerHTML = '<strong style="color:#e74c3c;">⚠ This is a spreadsheet URL, not the Apps Script Web App URL.</strong> Go to Extensions → Apps Script → Deploy → Manage deployments and copy the Web app URL (starts with https://script.google.com/macros/...).';
  } else if (url && !url.includes('script.google.com/macros')) {
    help.innerHTML = '<strong style="color:#e74c3c;">⚠ This doesn\'t look like an Apps Script URL.</strong> It should start with https://script.google.com/macros/...';
  } else {
    help.innerHTML = 'Paste the URL of your deployed Google Apps Script web app (starts with https://script.google.com/macros/...). <strong>This is NOT the spreadsheet URL</strong> — it\'s found in Apps Script → Deploy → Manage deployments.';
  }
}
...
function saveSettings() {
  ...
  if (sheetsUrl) {
    localStorage.setItem('tg_sheets_url', sheetsUrl);
    try { localStorage.setItem('comprehension_hub_sheet_url', sheetsUrl); } catch(e) {}
  } else {
    localStorage.removeItem('tg_sheets_url');
  }
  updateSettingsBadges();
  checkReadiness();
}
...
function setTestType(type) {
  testType = type;
  const isLC = type === 'language-conventions';

  // Update description
  document.getElementById('uploadDescription').textContent = isLC
    ? 'Upload PDF files of the Language Conventions test paper and answer key.'
    : 'Upload PDF files of the reading passage, question paper, and answer key. You can upload them as separate files or as a single combined document.';

  // Show/hide paste passage area
  document.getElementById('pastePassageArea').style.display = isLC ? 'none' : '';

  // Re-render file list with updated label options
  renderFileList();
  checkReadiness();
}
...
function addFile(file) {
  ...
  if (fileSizeMB > 50) {
    showToast('File too large (' + fileSizeMB.toFixed(1) + ' MB). Max is 50 MB per PDF.', 'error');
    return;
  }
  if (fileSizeMB > 10) {
    showToast('Large file (' + fileSizeMB.toFixed(1) + ' MB) — processing may take longer.', 'info');
  }

  // Auto-assign label
  let label = 'Combined Document';
  if (testType === 'language-conventions') {
    if (uploadedFiles.length === 0) label = 'Test Paper';
    else if (uploadedFiles.length === 1) label = 'Answer Key';
    else label = 'Combined Document';
  } else {
    ...
    html += '<div class="file-item">' +
      ...
      '<div style="display:flex;align-items:center;gap:8px;">' +
        '<select class="upload-label-select" onchange="changeFileLabel(' + i + ', this.value)">' +
          (testType === 'language-conventions'
            ? '<option value="Test Paper"' + (entry.label === 'Test Paper' ? ' selected' : '') + '>Test Paper</option>' +
...
function checkReadiness() {
  ...
  const pastePassageEl = document.getElementById('pastePassage');
  const hasPassage = pastePassageEl && pastePassageEl.value.trim().length > 0;
...
function buildReadingComprehensionPrompt(contextText) {
  return `You are a test parser. Analyze these reading comprehension test documents and extract the content as structured JSON.
...
function buildLanguageConventionsPrompt(contextText) {
  return `You are a NAPLAN Language Conventions test parser. Analyze these test documents and extract the content as structured JSON.

This is a Language Conventions test with two question types:
1. SPELLING questions (typically Q1-Q25): A sentence contains a misspelled word. The student must write the correct spelling.
   - Some questions have the misspelled word circled/highlighted in the original — the student writes the correct spelling
   - Other questions do NOT highlight the misspelled word — the student must find it themselves and write the correct spelling
   - For ALL spelling questions, include the FULL sentence with the misspelled word wrapped in <strong> tags
   - Set type to "spelling" and correctAnswer to the correctly-spelled single word

2. GRAMMAR/PUNCTUATION MCQ questions (typically Q26-Q51): Standard multiple choice with 4 options (a,b,c,d).
   - Topics include: word choice, punctuation, apostrophes, verb tense, sentence combining, parts of speech, commas, capitals
   - Some questions include embedded context (e.g. a poster, table, or letter extract) — include this as part of the question text using appropriate HTML (use <br> for line breaks, <em> for emphasis, <table> for tables)

Return a JSON object with this exact structure:
{
  "testTitle": "string - e.g. NAPLAN 2012 Language Conventions Year 5",
  "testType": "language-conventions",
  "sections": [
    {
      "id": 1,
  ...
  "questions": [
    {
      "num": 1,
      "sectionId": 1,
      "text": "The referee blew the whistle to <strong>resstart</strong> the game.",
      "type": "spelling",
    ...
    },
    {
      "num": 26,
      "sectionId": 2,
      "text": "Which word completes this sentence correctly?<br><em>Before I walk across the street, I look in both directions to make sure I can cross ___.</em>",
      "type": "mc",
      "options": {
        "a": "safe",
        "b": "safest",
...
Important rules for SPELLING questions:
- The text must contain the FULL SENTENCE as shown in the test
- Wrap the misspelled word in <strong> tags: "The <strong>bycicle</strong> was broken."
- For questions where the mistake is NOT highlighted in the original, still wrap the misspelled word in <strong> tags (look at the answer key to identify which word is misspelled)
- correctAnswer is the SINGLE correctly-spelled word (e.g., "bicycle"), NOT the full sentence
- correctAnswer should be lowercase unless it is a proper noun

Important rules for MCQ questions:
- Always extract exactly 4 options (a, b, c, d)
- correctAnswer should be the lowercase letter (a, b, c, or d)
- If a group of questions shares context (like a poster or passage), include that context as HTML at the start of the first question's text, and reference it in subsequent questions
- For questions about sentence punctuation, include the full sentences as options

General rules:
- Use 2 sections: "Spelling" for all spelling questions, "Grammar & Punctuation" for all MCQ questions — adjust if the test structure differs
- sections.passage should be empty string "" for all sections
- Number questions sequentially as they appear in the original test (skip practice questions P1-P4)
- If the answer key is provided, use it to set correctAnswer. If not, use your best judgement.
- Do NOT include any markdown code fences in the output, just pure JSON
- Make sure every question has a valid sectionId matching one of the sections
...
async function processWithAI() {
  ...
  showSpinner('Processing with Gemini AI...', 'This may take 30-90 seconds for large documents');
  ...
  try {
    ...
    const prompt = testType === 'language-conventions'
      ? buildLanguageConventionsPrompt(contextText)
      : buildReadingComprehensionPrompt(contextText);

    parts.push({ text: prompt });

    // Try models in order: 2.5-flash (best), 2.0-flash (legacy, shutting down Mar 2026), 2.5-flash-lite (lightweight fallback)
    ...
    for (const model of models) {
      ...
      // If quota/rate limit error or model not found, try next model
      if (response.status === 429 || response.status === 503 || response.status === 404) {
        const reason = response.status === 404 ? 'not available' : 'quota exceeded';
        showToast(model + ' ' + reason + ', trying next model...', 'info');
        continue;
      }
    ...
    if (!response.ok) {
      throw new Error(lastError + '\n\nTip: If this is a quota error, wait a minute and try again. The free Gemini key allows ~15 requests per minute.');
    }
    ...
    // Auto-detect test type from AI response
    if (parsed.testType === 'language-conventions') {
      testType = 'language-conventions';
      document.querySelector('input[name="testType"][value="language-conventions"]').checked = true;
    }
...
function populateEditor() {
  ...
  const defaultTitle = testType === 'language-conventions' ? 'Language Conventions Test' : 'Reading Comprehension Test';
  document.getElementById('editTestTitle').value = extractedData.testTitle || defaultTitle;
...
function renderSectionsEditor() {
  ...
  extractedData.sections.forEach((sec, si) => {
    ...
    const showPassage = testType !== 'language-conventions';
...
function renderQuestionCard(q, globalIndex) {
  ...
  // Build type dropdown options based on test type
  let typeOptionsHtml = '<option value="mc"' + (q.type === 'mc' ? ' selected' : '') + '>Multiple Choice</option>';
  if (testType === 'language-conventions' || q.type === 'spelling') {
    typeOptionsHtml += '<option value="spelling"' + (q.type === 'spelling' ? ' selected' : '') + '>Spelling</option>';
  }
  typeOptionsHtml += '<option value="written"' + (q.type === 'written' ? ' selected' : '') + '>Written Response</option>';
  ...
  let html = '<div class="question-card" data-q-index="' + globalIndex + '">' +
    ...
    '<div class="question-card-header">' +
      ...
      '<select onchange="changeQuestionType(' + globalIndex + ', this.value)">' + typeOptionsHtml + '</select>' +
    '</div>' +
  ...
  if (q.type === 'spelling') {
    // Spelling question: show text input for correct answer
    html += '<div class="spelling-answer-editor">' +
      '<label style="font-size:0.85rem;font-weight:600;color:var(--text);margin-bottom:4px;display:block;">Correct Spelling</label>' +
      '<input type="text" class="q-text-input" value="' + escapeAttr(q.correctAnswer || '') + '" ' +
        'onchange="updateQuestion(' + globalIndex + ', \'correctAnswer\', this.value)" ' +
        'placeholder="e.g. restart" style="max-width:300px;font-weight:600;color:var(--correct);">' +
      '<p class="spelling-hint">The single correctly-spelled word. Marking is case-insensitive.</p>' +
    '</div>';
  } else if (q.type === 'mc' || (q.type !== 'written' && q.type !== 'spelling')) {
    // MC question: show options
  ...
  // Written type: no options editor shown
...
function changeQuestionType(globalIndex, type) {
  ...
  if (type === 'written') {
    ...
    // Keep existing correctAnswer if it's a word; clear if it was a letter or 'written'
    if (q.correctAnswer === 'written' || ['a','b','c','d'].includes(q.correctAnswer)) {
      q.correctAnswer = '';
    }
    delete q.options;
  } else {
    ...
    if (q.correctAnswer === 'written' || (!['a','b','c','d'].includes(q.correctAnswer) && q.correctAnswer)) {
      q.correctAnswer = 'a';
    }
  }
  renderSectionsEditor();
...
function addQuestion(sectionIndex) {
  ...
  const defaultType = testType === 'language-conventions' ? 'spelling' : 'mc';
  const newQ = {
    num: nextNum,
    sectionId: sec.id,
    text: '',
    type: defaultType,
    correctAnswer: defaultType === 'spelling' ? '' : 'a'
  };
  if (defaultType === 'mc') {
    newQ.options = { a: '', b: '', c: '', d: '' };
  }
  extractedData.questions.push(newQ);
...
function addSection() {
  ...
  extractedData.sections.push({
    id: nextId,
    title: 'New Section',
    passage: testType === 'language-conventions' ? '' : '<p>Enter the reading passage here...</p>'
  });
...
function goToStep(step) {
  ...
  if (step === 4 && extractedData) {
    const defaultTitle = testType === 'language-conventions' ? 'Language Conventions Test' : 'Reading Comprehension Test';
    const title = document.getElementById('editTestTitle').value || extractedData.testTitle || defaultTitle;
    document.getElementById('genTitle').value = title;

    // Set default duration based on test type
    const currentDuration = parseInt(document.getElementById('genDuration').value) || 50;
    if (currentDuration === 50 && testType === 'language-conventions') {
      document.getElementById('genDuration').value = 40;
    } else if (currentDuration === 40 && testType === 'reading') {
      document.getElementById('genDuration').value = 50;
    }

    const isLC = testType === 'language-conventions';
    const spellingCount = extractedData.questions.filter(q => q.type === 'spelling').length;
    const mcCount = extractedData.questions.filter(q => q.type === 'mc' || (q.type !== 'written' && q.type !== 'spelling')).length;
    const writtenCount = extractedData.questions.filter(q => q.type === 'written').length;

    let summaryRows = '<p><strong>Sections:</strong> ' + extractedData.sections.length + '</p>';
    if (isLC) {
      summaryRows += '<p><strong>Spelling Questions:</strong> ' + spellingCount + '</p>';
      summaryRows += '<p><strong>Grammar &amp; Punctuation MCQ:</strong> ' + mcCount + '</p>';
      summaryRows += '<p><strong>Total Questions:</strong> ' + extractedData.questions.length + ' <span style="color:var(--correct);">(all auto-marked)</span></p>';
    } else {
      summaryRows += '<p><strong>Multiple Choice Questions:</strong> ' + (mcCount + spellingCount) + '</p>';
      summaryRows += '<p><strong>Written Response Questions:</strong> ' + writtenCount + '</p>';
      summaryRows += '<p><strong>Total Questions:</strong> ' + extractedData.questions.length + '</p>';
    }
...
function generateTest() {
  ...
  const defaultTitle = testType === 'language-conventions' ? 'Language Conventions Test' : 'Reading Comprehension Test';
  const testTitle = document.getElementById('genTitle').value.trim() || defaultTitle;
  const duration = parseInt(document.getElementById('genDuration').value) || (testType === 'language-conventions' ? 40 : 50);
  const sheetsUrl = document.getElementById('sheetsWebAppUrl').value.trim() || (localStorage.getItem('comprehension_hub_sheet_url') || '');
  ...
  sections.forEach((sec, si) => {
    ...
    secQuestions.forEach(q => {
      if (q.type === 'written') {
        ...
        sectionsJS += '      { num:' + q.num + ', text:' + JSON.stringify(q.text) + ', type:"spelling" },\n';
      } else {
  ...
  questions.forEach(q => {
    if (q.type === 'written') {
      ...
      // Store correct spelling word, normalized lowercase
      answersJS += '  ' + q.num + ':' + JSON.stringify((q.correctAnswer || '').toLowerCase().trim()) + ',\n';
    } else {
  ...
  // Build the spelling question numbers array
  const spellingQuestions = questions.filter(q => q.type === 'spelling');
  const spellingNums = spellingQuestions.map(q => q.num);
  const spellingNumsJS = 'const SPELLING_QUESTIONS = [' + spellingNums.join(',') + '];\n';

  // Build question texts for Results Analyser (Q1, Q2, ... -> text)
  const questionTexts = {};
  questions.forEach(q => {
    questionTexts['Q' + q.num] = (q.text || '').trim();
  });
  ...
  const html = buildTestHTML({
    ...
    questionTexts,
    appsScriptCode,
    isLanguageConventions: testType === 'language-conventions'
  });
  ...
  // Auto-register on the landing page's test library (shared localStorage)
  try {
    const LIBRARY_KEY = 'comprehension_hub_tests';
    let tests = JSON.parse(localStorage.getItem(LIBRARY_KEY) || '[]');
    // Don't add duplicate if filename already exists
    if (!tests.some(t => t.file === filename)) {
      tests.push({
        id: 'test-' + Date.now(),
        name: title,
        file: filename,
        ...
        questions: totalQ,
        duration: duration,
        tag: testType === 'language-conventions' ? 'language-conventions' : 'comprehension'
      });
      localStorage.setItem(LIBRARY_KEY, JSON.stringify(tests));
    }
  } catch(e) { /* localStorage not available */ }

  showToast('Test file downloaded! It will also appear on your landing page.', 'success');
}

function generateAppsScriptCode(questions) {
  // Universal Apps Script: one tab per test (testName), sanitised for sheet names
  ...
  try {
    ...
    if (sheet.getLastRow() === 0) {
      ...
      // Write question text row so Results Analyser can show questions when expanding
      if (data.questionTexts && typeof data.questionTexts === 'object') {
        var row2 = [];
        for (var c = 0; c < headers.length; c++) {
          row2.push(data.questionTexts[headers[c]] || '');
        }
    ...
    var spellingQs = data.spellingQuestions || [];
    ...
    for (var i = 1; i <= totalQ; i++) {
      ...
      } else if (spellingQs.indexOf(i) > -1) {
        // Spelling: case-insensitive comparison, show original casing
        var isRight = ans.trim().toLowerCase() === correct.trim().toLowerCase();
        row.push(isRight ? '\\u2713 ' + ans : '\\u2717 ' + (ans || '-') + ' (' + correct + ')');
      } else {
...
function doGet(e) {
  // Return all sheet tab names and their data for the analyser (row 1 = headers, row 2 = question text, row 3+ = students)
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  ...
  for (var s = 0; s < sheets.length; s++) {
    ...
    var headers = data[0];
    var hasQuestionRow = data.length >= 2 && (data[1][0] === '' || data[1][0] === null || data[1][0] === undefined || !/\\d{1,2}[\\/\\-]\\d{1,2}/.test(String(data[1][0])));
    var questionRow = hasQuestionRow ? data[1] : [];
    var dataRows = hasQuestionRow ? data.slice(2) : data.slice(1);
    var questionTexts = {};
    for (var c = 0; c < headers.length; c++) {
...
function buildTestHTML(config) {
  const { title, duration, totalQ, totalMC, sheetsUrl, sectionsJS, answersJS, writtenNumsJS, writtenNums, spellingNumsJS = 'const SPELLING_QUESTIONS = [];', spellingNums = [], questionTexts = {}, appsScriptCode, isLanguageConventions = false } = config;
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .start-container select {
    width: 100%; padding: 14px 16px; font-size: 1.1rem; border: 2px solid var(--border);
    border-radius: 8px; margin-bottom: 12px; transition: border-color 0.2s; background: var(--card);
  }
  .start-container select:focus { border-color: var(--primary-light); outline: none; }
  ...
  ${isLanguageConventions ? `
  /* Language Conventions: single-column layout, no passage panel */
  ...
  .spelling-input { margin-top: 8px; }
  .spelling-input input[type="text"] {
    width: 100%; max-width: 300px; padding: 10px 14px; border: 2px solid var(--border);
    border-radius: 8px; font-size: 1.1rem; font-family: inherit; transition: border-color 0.2s;
  }
  .spelling-input input[type="text"]:focus { border-color: var(--primary-light); outline: none; }
  .spelling-input .spell-hint { font-size: 0.8rem; color: #888; margin-top: 4px; }
  .question-text strong { color: var(--incorrect); text-decoration: underline wavy var(--incorrect); }
  ...
  @media (max-width: 900px) {
    ...
    .spelling-input input[type="text"] { max-width: 100%; }
  }
</style>
</head>
<body>
...
<div id="startScreen" class="screen active">
  <div class="start-container">
    ...
    <!-- Keep as <select> for consistency with library tests (2012, 2016, 2025); do not replace with text input -->
    <select id="studentClass" class="class-input" aria-label="Select your class">
      <option value="">Select your class</option>
      <option value="5A">5A</option>
      <option value="5EG">5EG</option>
...
const QUESTION_TEXTS = ${JSON.stringify(questionTexts)};
...
function getSheetUrl() {
  return localStorage.getItem('test_sheet_url_' + location.pathname) || localStorage.getItem('comprehension_hub_sheet_url') || EMBEDDED_SHEET_URL || '';
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  classInput.addEventListener('change', checkReady);
  ...
  var savedUrl = localStorage.getItem('test_sheet_url_' + location.pathname) || localStorage.getItem('comprehension_hub_sheet_url');
...
function buildQuestionsForSection(sec) {
  ...
  sec.questions.forEach(function(q) {
    if (q.type === 'written') {
      ...
      var savedVal = studentAnswers['q' + q.num] || '';
      var hintText = q.text.indexOf('<strong>') > -1
        ? 'Write the correct spelling of the highlighted word'
        : 'Find the misspelled word and write its correct spelling';
      html += '<div class="question" id="question-' + q.num + '">' +
        '<div class="question-text"><span class="q-num">Q' + q.num + '.</span> ' + q.text + '</div>' +
        '<div class="spelling-input">' +
          '<input type="text" id="answer-' + q.num + '" value="' + escapeHtml(savedVal) + '" ' +
            'placeholder="Type the correct spelling" ' +
            'oninput="saveSpellingAnswer(' + q.num + ', this.value)" ' +
            'autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false">' +
          '<div class="spell-hint">' + hintText + '</div>' +
        '</div></div>';
    } else {
      var optionsHtml = '';
      var keys = ['a', 'b', 'c', 'd'];
...
function showSection(idx) {
  ...
  var passageEl = document.getElementById('passagePanel');
  if (passageEl && passageEl.style.display !== 'none') {
    passageEl.innerHTML = sec.passage;
  }
  ...
  if (passageEl && passageEl.style.display !== 'none') passageEl.scrollTop = 0;
  updateProgress();
}
...
function saveSpellingAnswer(qNum, value) {
  if (value.trim()) {
    studentAnswers['q' + qNum] = value.trim();
  } else {
    delete studentAnswers['q' + qNum];
...
function submitTest() {
  ...
  for (var i = 1; i <= TOTAL_QUESTIONS; i++) {
    ...
    // Spelling questions: case-insensitive, trimmed word comparison
    if (SPELLING_QUESTIONS.indexOf(i) > -1) {
      var normAnswer = answer ? answer.trim().toLowerCase() : '';
      var normCorrect = correct.trim().toLowerCase();
      var isCorrect = normAnswer === normCorrect;
      if (isCorrect) score++;
      results.push({ num: i, answer: answer ? answer.trim() : '\\u2014', correct: correct, isCorrect: isCorrect, type: 'spelling' });
      continue;
    }
...
function sendToSheets(score, percentage, total) {
  ...
  var payload = {
    ...
    spellingQuestions: SPELLING_QUESTIONS,
    answers: studentAnswers,
    correctAnswers: correctAnswersForSheet,
    questionTexts: typeof QUESTION_TEXTS !== 'undefined' ? QUESTION_TEXTS : {}
  };
...
function submitViaForm(url, payload, statusEl, score, percentage, total) {
  ...
  statusEl.className = 'sheets-status sending';
  statusEl.textContent = 'Sending results to your teacher...';

  // Prefer fetch so we can show real success/error from the server
  fetch(url, {
    method: 'POST',
    redirect: 'follow',
    ...
    .then(function(r) {
      try {
        ...
        statusEl.textContent = 'Server error: ' + (data.message || r.text || 'Unknown');
      } catch (e) {
        statusEl.className = 'sheets-status error';
        statusEl.textContent = r.status === 200 ? 'Results may have been sent; check the spreadsheet.' : 'Connection failed (' + r.status + '). Check the Web App URL and deployment access.';
      }
    })
    .catch(function() {
      // Fallback: form submit (no response feedback); user can check the sheet
      ...
      form.submit();
      statusEl.className = 'sheets-status success';
      statusEl.textContent = 'Results sent (if the connection worked, they will appear in the spreadsheet).';
...
function saveConfig() {
  ...
  if (url) {
    ...
    try { localStorage.setItem('comprehension_hub_sheet_url', url); } catch(e) {}
